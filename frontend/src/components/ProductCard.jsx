...
function ProductCard({ product }) {
  ...
  const offerPrice = product.precioOferta ?? product.priceOferta ?? 0
    ...
    <Link to={`/productos/${id}`} className="product-card">
      ...
      <div className="product-card__body">
        {product.marca?.nombre && (
          <div className="product-card__brand">{product.marca.nombre}</div>
        )}
        {categories.length > 0 && (
        ...
        <p className="product-card__price">
          {hasOffer ? (
            <>
              <span className="product-card__price--old">${Number(price).toLocaleString('es-AR')}</span>
              <span>${Number(displayPrice).toLocaleString('es-AR')}</span>
            </>
          ) : (
